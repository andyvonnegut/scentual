...
- **Left:** big serif name with a clickable favorite star immediately to its right, house link (sized `text-sm` uppercase label), `SaveControls`, `RatingsControlGroup` (three independent 1..5 personal scales shown in the order Overall, Projection, Design; Projection uses the old atomizer glyph, Overall uses hearts, Design uses a traditional eau de parfum bottle), personal tags (always shown — `Your notes` and `Themes`, attachable even when the perfume isn't in Collection or Wanted), a separate read-only `Store notes` section, a tabbed narratives section (`PerfumeNarrativeTabs` — one tab per retailer showing the scraped `source_description`, plus a "Yours" tab for signed-in users that renders / edits a personal markdown narrative stored on `personal_perfumes.personal_narrative`). Selecting a source tab reveals only that retailer's prose so multi-source perfumes don't stack. The "Yours" tab opens in read mode with an Edit button; edit mode swaps to a textarea with a markdown toolbar (Bold / Italic / H1 / H2 / bulleted list / numbered list / quote / link), ⌘ Enter to save, Esc to cancel. Save calls `updatePersonalNarrative`; rendering uses the `marked` library with default `html=false` so user-pasted HTML is escaped. No "open source" link here — it's on the availability row instead.
- **Right aside (stacked):**
...
Saved-perfumes page. Header shows only **Collection** with the perfume count, with no micro-label above it. Tab bar (`LibraryFilterTabs`): **All** (default) / **Owned** / **Desired** / **Sniffed** / **Curious** via `?filter=`. The active tab uses the `accent-strong` underline from the perfume-page source tabs plus the semibold `accent-strong` text treatment from the top nav. Legacy filter values (`collection` / `wanted` / `both`) redirect to their new equivalents (`owned` / `desired` / `all`). Top card is `AddPerfumeSearch` (typeahead into `/api/catalog/search`, four buttons per hit to add to Owned / Desired / Sniffed / Curious). When the catalog search returns zero matches, the card swaps in a prominent "Add it as a custom scent" link that carries the typed query into `/collection/add?name=...`; an always-visible muted "Can't find it? Add a custom scent →" link sits below the input. Grid of `SavedCard`s (perfume, house, favorite star beside the perfume name, Owned / Desired / Sniffed / Curious chips, size_owned, personal note, store notes, personal-note chips, theme tags, compact three-row `RatingsControlGroup` with inline right-side labels only, compact `SaveControls`). Data: `getSavedPerfumes(filter)`.

### `/collection/add` — Add a custom scent
Lets the user create a perfume that isn't in the catalog and drop it into Owned, Desired, Sniffed, or Curious. Two-stage typeahead form (`AddCustomScentForm`): (1) **House** — typeahead-with-create against `/api/manufacturers/search`. Picking an existing house reuses its `manufacturer_id`; pressing Enter on an empty-results state stages a new house (the server action does the actual insert). (2) **Perfume name** — disabled until a house is locked. Once a house is selected, the field becomes a manufacturer-scoped catalog typeahead (`/api/catalog/search?q=...&manufacturer_id=`); if a real catalog hit appears, the user sees a "Use this" CTA that calls the existing `toggleOwned/Desired/Sniffed/Curious` action and skips the form. (3) **List kind** — Owned / Desired / Sniffed / Curious pill picker (Owned default). On submit, calls `createUserPerfume` (`app/actions/user-perfumes.ts`). Custom rows are private to the creator — they appear in the creator's `/collection`, `/perfumes/[manufacturer]/[slug]`, and `/browse`, but other users see only canonical rows. If the scraper later finds the same scent (matching `(manufacturer.slug, perfume.slug)`), `ingestOne` clears the user-submitted flags in place so the row becomes canonical without changing its id, leaving every `personal_perfumes` link intact.
...
### `library.ts`
- `toggleOwned(perfumeId, next)` — upserts `personal_perfumes`, flips `in_owned`, stamps `added_to_owned_at`. When all four list flags become false, deletes the row only if it carries no tags and no `size_owned_text` / `personal_note`; otherwise leaves a bare row so attached tags / notes survive.
- `toggleDesired(perfumeId, next)` — same for `in_desired` / `added_to_desired_at`.
- `toggleSniffed(perfumeId, next)` — same for `in_sniffed` / `added_to_sniffed_at`.
- `toggleCurious(perfumeId, next)` — same for `in_curious` / `added_to_curious_at`.
- `toggleFavorite(perfumeId, next)` — upserts `personal_perfumes.favorite`. Creates a bare row if favoriting a perfume with no other personal state; deletes that bare row when unfavoriting would leave no owned/desired/sniffed/curious flags, notes, tags, ratings, or other personal data behind.
...
- `updatePersonalNarrative(perfumeId, narrative)` — upserts `personal_perfumes.personal_narrative` (markdown source). Creates a bare row when first writing on a perfume that has no other personal state; clears the field by passing `null` and deletes the bare row only if no other personal data remains and all four list flags are false. Powered by the "Yours" tab in `PerfumeNarrativeTabs`.
...
### Personal library
- **`personal_perfumes`** — `id, user_id uuid→auth.users CASCADE, perfume_id→ CASCADE, in_owned, in_desired, in_sniffed, in_curious, favorite default false, size_owned_text?, personal_note?, personal_narrative? (markdown source for the "Yours" tab on the perfume detail page; rendered client-side via marked, default html=false escaping prevents raw HTML from reaching the DOM), projection_rating? smallint CHECK (1..5), overall_rating? smallint CHECK (1..5), design_rating? smallint CHECK (1..5), added_to_owned_at?, added_to_desired_at?, added_to_sniffed_at?, added_to_curious_at?, updated_at`. `UNIQUE(user_id, perfume_id)` (the original `UNIQUE(perfume_id)` was swapped out in `20260419220002_user_id_personal.sql`). Partial indexes on each of the four list flags, plus `(user_id)` for RLS-filtered reads. Bare rows (all four flags false) are allowed so favorite state, tags, notes, or ratings can exist without the perfume being in any list — the original `CHECK (in_collection OR in_wanted)` was dropped in `20260419020001_allow_bare_personal_perfumes.sql`. The taxonomy rename from `in_collection / in_wanted` to `in_owned / in_desired` plus the new `in_sniffed` flag landed in `20260420040001_rename_collection_wanted_and_add_sniffed.sql`; the fourth `in_curious` flag was added in `20260518000001_add_curious.sql`.
...
- **`SaveControls.tsx`** — *client.* Four toggle buttons (Owned / Desired / Sniffed / Curious) with `useTransition`. `compact` variant for `SavedCard`.
- **`LibraryFilterTabs.tsx`** — *server.* URL-driven tab bar for `/collection?filter=`. Combines the `PerfumeNarrativeTabs` underline frame with the `ShellNav` active-link styling (semibold + `accent-strong`). Tabs: All / Owned / Desired / Sniffed / Curious.
...
- `collection/_components/AddPerfumeSearch.tsx` — client, typeahead → `/api/catalog/search`, one-click add to Owned / Desired / Sniffed / Curious.
