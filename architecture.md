...
- **`LibraryFilterTabs.tsx`** — *server.* URL-driven tab bar for `/collection?filter=`. Combines the `SourceDescriptionTabs` underline frame with the `ShellNav` active-link styling (semibold + `accent-strong`). Tabs: All / Owned / Desired / Sniffed.
