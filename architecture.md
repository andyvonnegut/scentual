...
- a single search line (`q`) that tokenizes whitespace and requires every typed word to match somewhere across perfume name, manufacturer name, canonical accords attached by the store, or canonical notes attached by the user
...
- **Left:** big serif name with a clickable favorite star immediately to its right when signed in, house link (sized `text-sm` uppercase label), `SaveControls`, `RatingsControlGroup` (three independent 1..5 personal scales shown in the order Overall, Projection, Design; Projection uses the old atomizer glyph, Overall uses hearts, Design uses a traditional eau de parfum bottle), personal tags (always shown to signed-in users — `Your notes` and `Themes`, attachable even when the perfume isn't in Collection or Wanted), a separate read-only `Accords` section for retailer-derived scent descriptors, a `Fragrantica` metadata card (year, gender, Fragrantica rating/votes, longevity, sillage, grouped top/middle/base notes, canonical description, external source link), and a tabbed source-descriptions section (`SourceDescriptionTabs` — one tab per retailer; selecting a tab reveals only that retailer's `source_description`, so multi-source perfumes don't stack prose blocks). No "open source" link here — it's on the availability row instead.
- **Right aside (stacked):**
...
Data: `getPerfumeByManufacturerAndSlug` returns the public perfume tree plus Fragrantica metadata columns on `perfumes` (manufacturer, perfume_notes, perfume_listings → retailer + variants, Fragrantica fields). User-scoped pieces (`personal_perfumes`, `journal_entries`) are fetched separately via `getPersonalPerfumeByPerfumeId` and `listJournalEntriesForPerfume`, both of which filter by `user_id = auth.uid()` and return `null`/`[]` for anonymous visitors. Then `getPriceHistory(variantId)` / `getStockHistory(variantId)` are fanned out in parallel for every variant.

When a user is not signed in, the page still renders the catalog data, accords, Fragrantica metadata, source-description tabs, availability, and history, but swaps the personal controls for a "Sign in to save, rate, and journal" CTA and omits `JournalSection`.

### `/collection` — Collection
Saved-perfumes page renamed from Library to Collection. Header shows only **Collection** with the perfume count, with no micro-label above it. Filter pills: **All Saved** (default) / **Collection** / **Wanted** / **Both** via `?filter=`. Top card is `AddPerfumeSearch` (typeahead into `/api/catalog/search`, two buttons per hit to add to Collection or Wanted). Grid of `SavedCard`s (perfume, house, favorite star beside the perfume name, Collection/Wanted chips, size_owned, personal note, accords, personal-note chips, theme tags, compact three-row `RatingsControlGroup` with inline right-side labels only, compact `SaveControls`). Data: `getSavedPerfumes(filter)`.
...
- `[source]` ∈ `ministryofscent | luckyscent | fragrantica`.
...
**Ingest:** Vercel Cron → `/api/cron/scrape/[source]` → `runScrape` → either (a) retailer scraper `crawl()` async iterable → `ingestOne` per item (upserts + history rows + listing-level accord sync) → `markStaleListingsInactive` → `rebuildCanonicalNotes`, or (b) Fragrantica metadata sync via PerfumAPI → direct perfume upserts on the canonical catalog, then write `scrape_runs`.
...
- **`note-sync.mjs`** — exact listing-level accord sync (`perfume_source_notes`) plus canonical note rebuild across active listings. Rebuild deletes stale inactive listing note rows, repopulates `source_notes` and `perfume_notes`, then prunes unused rows from `notes`.
...
- **`fragrantica.ts`** — metadata-only sync against the external PerfumAPI host (`PERFUMAPI_BASE_URL`, default `https://perfumapidatabase.onrender.com`). Warms up `/stats`, pages `/perfumes?limit=100&offset=N`, normalizes trailing brand suffixes in upstream names (for rows like `Black Afgano Nasomatto`), and merges by `fragrantica_url` first or `(manufacturer.slug, perfume.slug)` second. Creates new canonical perfumes when unmatched, but does not create listings or variants.
...
### Runner (`runner.ts`)
`runScrape(sourceSlug, runType)` creates a `scrape_runs` row (`status='running'`). Retailer sources loop `scraper.crawl()` calling `ingestOne`, run stale deactivation, rebuild canonical note tables, then finalize the run row. `fragrantica` skips retailer lookup and stale deactivation, runs the PerfumAPI metadata sync directly against `perfumes`, and finalizes the run row with the same counts/error shape.
...
- `fragrantica` — `40 0 * * *` (00:40 UTC / 8:40 PM EDT while DST is in effect)
...
- **`perfumes`** — `id, manufacturer_id→, name, slug, canonical_description?, release_year?, gender?, notes_top text[] DEFAULT '{}', notes_middle text[] DEFAULT '{}', notes_base text[] DEFAULT '{}', fragrantica_rating?, fragrantica_votes?, fragrantica_longevity?, fragrantica_sillage?, fragrantica_url?, fragrantica_last_synced_at?`. `UNIQUE(manufacturer_id, slug)` plus a partial unique index on `fragrantica_url where not null`. Indexed on `manufacturer_id`, `created_at desc`, `updated_at desc`.
...
- **`Chip.tsx`** — CVA. Variants: `store` (read-only store accords / source chips), `fragrance-note` (accent-toned, for fragrance notes), `theme` (neutral, for theme tags). Sizes: `sm | md`.
...
- Store accords (retailer scraper output) and personal note attachments both point at the shared canonical `notes` table, but they are stored in different join tables and still render in separate UI sections.
- Canonical store-accord attachments are an exact mirror of explicit note blocks on active product pages. If a scrape-derived accord disappears from every active listing, the rebuild prunes it from `source_notes` and `perfume_notes`, but canonical `notes` rows may still remain when users have promoted or attached them personally. Fragrantica top/middle/base notes are stored directly on `perfumes` for display and are not merged into `perfume_notes`.
