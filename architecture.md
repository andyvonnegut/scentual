...
- **`RatingControl.tsx`** — *client.* Contains `RatingControl` (single personal rating row) and `RatingsControlGroup` (Projection / Overall / Design bundle). Each scale is a 1..5 click-to-clear widget with hover preview, optimistic update via `useTransition`, and rollback on error. Projection keeps the old-fashioned atomizer SVG, Overall uses a heart SVG, and Design uses a traditional eau de parfum bottle SVG. Shared props include `perfumeId`, `size ("sm" | "md")`, and initial ratings; grouped usage passes `{ projection, overall, design }`.
