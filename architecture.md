...
### `library.ts`
- `toggleOwned(perfumeId, next)` — upserts `personal_perfumes`, flips `in_owned`, stamps `added_to_owned_at`. When all three list flags become false, deletes the row only if it carries no tags and no `size_owned_text` / `personal_note`; otherwise leaves a bare row so attached tags / notes survive.
...
### Personal library
- **`personal_perfumes`** — `id, user_id uuid→auth.users CASCADE, perfume_id→ CASCADE, in_owned, in_desired, in_sniffed, favorite default false, size_owned_text?, personal_note?, personal_narrative? (markdown source for the "Yours" tab on the perfume detail page; rendered client-side via marked, default html=false escaping prevents raw HTML from reaching the DOM), projection_rating? smallint CHECK (1..5), overall_rating? smallint CHECK (1..5), design_rating? smallint CHECK (1..5), added_to_owned_at?, added_to_desired_at?, added_to_sniffed_at?, updated_at`. `UNIQUE(user_id, perfume_id)` (the original `UNIQUE(perfume_id)` was swapped out in `20260419220002_user_id_personal.sql`). Partial indexes on each of the three list flags, plus `(user_id)` for RLS-filtered reads. Bare rows (all three flags false) are allowed so favorite state, tags, notes, or ratings can exist without the perfume being in any list — the original `CHECK (in_collection OR in_wanted)` was dropped in `20260419020001_allow_bare_personal_perfumes.sql`. The taxonomy rename from `in_collection / in_wanted` to `in_owned / in_desired` plus the new `in_sniffed` flag landed in `20260420040001_rename_collection_wanted_and_add_sniffed.sql`.
...
- **`SaveControls.tsx`** — *client.* Three toggle buttons (Owned / Desired / Sniffed) with `useTransition`. `compact` variant for `SavedCard`.
- **`LibraryFilterTabs.tsx`** — *server.* URL-driven tab bar for `/collection?filter=`. Combines the `PerfumeNarrativeTabs` underline frame with the `ShellNav` active-link styling (semibold + `accent-strong`). Tabs: All / Owned / Desired / Sniffed.
...
- `collection/_components/AddPerfumeSearch.tsx` — client, typeahead → `/api/catalog/search`, one-click add to Owned / Desired / Sniffed.
